...
def preflight(args):
    ...
    if subprocess.run(['virsh', '-c', 'qemu:///system', 'list'],
                      capture_output=True).returncode != 0:
        probs.append('libvirt not reachable without sudo (qemu:///system): '
                     'sudo usermod -aG libvirt $USER, then relogin')
    for tool in ('virt-install', 'cloud-localds', 'qemu-img'):
        if not shutil.which(tool):
            probs.append(f'{tool} not found — check-prereqs.sh names the apt package')
    return probs
