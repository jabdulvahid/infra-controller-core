#!/usr/bin/env python3
"""
nico-dev — Build Nico Docker images (ARM64) on Mac and push to local registry.
...
Usage:
  python3 build-dev-nico-mac.py <site> --tag <tag>
  python3 build-dev-nico-mac.py <site> --tag test1
...
def main():
    ...
    if subprocess.run(['docker', 'info'], capture_output=True).returncode != 0:
        ...
        print('  If you use colima:  colima start', file=sys.stderr)
        sys.exit(1)
    ...
    print('nico-dev — Build Nico (ARM64, Mac)')
    ...
    if not args.push_only:
        ...
        try:
            ...
            docker_build(
                ...
                label=f'nico:{args.tag} (ARM64 dev)',
            )
        finally:
            for name in copies:
    ...
    if args.skip_rest or not rest_enabled:
        ...
        if r.returncode != 0:
            print('\nError: docker buildx not available — required for the REST '
                  'image build.\n  With colima: brew install docker-buildx && '
    ...
    print(f'\n  Verify registry (Mac or VM):')
    print(f'    python3 {here}/ndev.py {args.site} registry verify')
    print(f'\n  First deploy (on Mac):')
    print(f'    python3 {here}/deploy-dev-nico.py {args.site} --tag {args.tag}')
    print(f'\n  Redeploy after code change (on Mac):')
    print(f'    python3 {here}/redeploy-dev-nico.py {args.site} --tag {args.tag}')
    print(f'{"="*55}')
